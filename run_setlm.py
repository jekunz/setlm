...
class SetLM_Model(nn.Module):
    def __init__(self, hidden_size, vocab_size, embedding_size, num_layers=1, init_range=(-0.1, 0.1), dropout=0.1, train_states=False):
        ...
        self.new_entity = nn.Parameter(torch.FloatTensor(num_layers, self.batch_size, hidden_size).to(device), requires_grad=True)
        self.no_entity = nn.Parameter(torch.FloatTensor(num_layers, self.batch_size, hidden_size).to(device), requires_grad=True)
    ...
    def init_h_e(self):
        ...
        self.h_e_m = torch.cat((self.no_entity, self.new_entity), dim=0)


    def weight_init(self, init_range):
        # initialize layer weights
        self.embedding_matrix.weight.data.uniform_(*init_range)
        self.new_entity.data.uniform_(*init_range)
        self.no_entity.data.uniform_(*init_range)
    ...
    def hard_soft_attn(self, query, values, hard_amplifier=10):
        """
            ...
            hard_amplifier: the highest value of the keys is multiplied by the hard_amplifier
                            before applying softmax. Aim is to create a de facto hard attention 
                            which still is differentiable. 
                            Setting hard_amplifier to '1' leads to full soft attention.
        """
        # ceating keys via matrix multiplication
        #print(query.size(), values.size())
        keys = torch.matmul(self.W_score(values), query.transpose(0, 1))
        max_key_index = keys.argmax().view(1, 1)
        # adding a factor to maximum key to archive pseudo hard-attention
        vector_factor = torch.ones_like(keys).scatter(0, max_key_index, hard_amplifier)
        # getting soft attention keys
        soft_keys = torch.nn.functional.softmax(keys*vector_factor, dim=0).squeeze()
    ...
    def train_forward(self, x, entity_target):
        ...
        if entity_target:
            if entity_target == 1:
                ...
                out = self.W1(torch.tanh(self.drop_layer( self.W2(torch.cat((hidden.squeeze(), entity_state.squeeze()), dim=0))) )).unsqueeze(0)   
            else:
    ...
    for epoch in range(1, epochs+1):
        ...
        for i_doc, doc in enumerate(corpus.gen()):
            ...
            for t in range(doc_num_tokens):
                ...
                if e_target > highest_entity:
                    highest_entity = e_target
                    e_target = new_entity_target
                # forward model
                x_out, e_out = model.train_forward(doc.X[t], e_target)
                e_pred = e_out.argmax()
            ...
            if status_interval and i_doc % status_interval == 0:
                ...
                z_recall = z_true_positive / max(count_E, 1)
                print(f'Doc {i_doc}/{len(corpus)-1}: X_loss {X_epoch_loss / epoch_tokens:0.3}, E_loss {E_epoch_loss / epoch_tokens:0.3}, E_acc {count_E_correct/count_E:0.3}, Z_prec {z_prec:0.3}, Z_recall {z_recall:0.3}')
        ...
        if optimizer:
            ...
            if eval_corpus:
                with torch.no_grad():
                    model.eval()
                    run_sem(model, eval_corpus, status_interval=None)
                    model.train()
