...
class EntityNLM(nn.Module):
    def __init__(self, vocab_size, embedding_size=256, hidden_size=256, num_layers=1, dropout=0.5, train_states=None, tied_embedding=None):
        ...
        self.output_layer = nn.Linear(hidden_size, vocab_size) # Linear
        ...
        # Wentity is the weight matrix for predicting entities using their continuous representa- tions
        ...
        self.W_delta = nn.Bilinear(hidden_size, hidden_size, 1) # Delta Matrix
    ...
    def add_new_entity(self, t=0.0):
        # appends a new entity including distance features
        # appending new embedding u to entity matrix
        self.entities = torch.cat((self.entities, self.initialize_entity_embedding()), dim=0)
        # creating respective distance features
    ...
    def update_entity_embedding(self, entity_index, h_t, t):
        ...
        # Updating entity embedding based with h_t using δ_t
    ...
    def get_dist_feat(self, t):
        # subtracting current time step from dist feature vector
        return self.dist_features - t

    def get_next_R(self, h_t):
        # predicting distribution for next R
    ...
    def register_predicted_entity(self, e_index):
        ...
        # there is a free slot in the entitiy set
        new_max = max(int(e_index), self.max_entity_index)
        self.max_entity_index = new_max

    def reset_state(self):
        # resets all entity states
        self.entities      = torch.tensor([], dtype=torch.float, device=device)
        self.dist_features = torch.tensor([], dtype=torch.float, device=device)
        self.max_entity_index = 0


def run_nlm(model, corpus, optimizer=None, epochs=1, eval_corpus=None, status_interval=25, str_pattern='{}_{}_epoch_{}.pkl', rz_amplifier=1, start_epoch=1):
    entity_offset = 1
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        for i_doc, doc in enumerate(corpus.gen()):
            ...
            # forwarding first token through Embedding and RNN
            # initialize states
            ...
            for t in range(doc.X.size(0)-1):      
                ...
                if current_L == 1:
                    ...
                    # adding division counter for R loss
                    r_div  += 1
                    ...
                    if next_R == 1:
                        ...
                        # setting e_current to entity embedding e_t-1
                    ...
                    # additionally it makes stats more compareable to deep-mind paper
                    E_dist = model.get_next_E(h_t, t)
                    count_E += 1
                    count_E_correct += int(E_dist.argmax() == next_E)
                    E_loss += torch.nn.functional.cross_entropy(E_dist, next_E.view(-1))
                ...
                if next_R == 1:
                    ...
                    # setting e_current to embedding e_t
                    e_current = model.get_entity_embedding(next_E)
